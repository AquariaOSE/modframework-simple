...
local ProFi = {}
local onDebugHook, sortByDurationDesc, sortByCallCount, getTime, scaleTime
...
function ProFi:setScaleTimeMethod( scaleTimeMethod )
	scaleTime = scaleTimeMethod
end
...
function ProFi:writeProfilingReportsToFile( reports, file )
	local totalTime = scaleTime(self.stopTime - self.startTime)
...
function ProFi:onFunctionReturn( funcInfo )
	...
	if funcReport.callTime then
		funcReport.timer = funcReport.timer + scaleTime(getTime() - funcReport.callTime)
	end
end
